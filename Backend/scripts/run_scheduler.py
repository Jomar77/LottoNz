...
def setup_windows_monthly_task():
    """Setup Windows Task Scheduler for monthly runs"""
    ...
    # Create monthly task (15th of each month at 5:00 PM)
    task_name = "LottoNZ_Monthly_Scraper"
    cmd = [
        "schtasks", "/create",
        "/tn", task_name,
    ...
    try:
        ...
        print(f"✅ Monthly task created: {task_name}")
        print(f"   Schedule: 15th of each month at 5:00 PM")
        print(f"   Script: {scheduler_script}")
        return True
    except subprocess.CalledProcessError as e:
        print(f"❌ Failed to create monthly task: {e.stderr}")
        return False
...
def setup_windows_tasks():
    ...
    # Setup monthly task
    monthly_success = setup_windows_monthly_task()
    print()
    ...
    # Summary
    print("=" * 70)
    if monthly_success and startup_success:
        print("✅ Both tasks created successfully!")
        print()
        print("Task Management Commands:")
        print("  View monthly task:   schtasks /query /tn LottoNZ_Monthly_Scraper /v /fo LIST")
        print("  View startup task:   schtasks /query /tn LottoNZ_Startup_Check /v /fo LIST")
        print("  Run monthly task:    schtasks /run /tn LottoNZ_Monthly_Scraper")
        print("  Delete monthly task: schtasks /delete /tn LottoNZ_Monthly_Scraper /f")
        ...
        if not monthly_success:
            print("   - Monthly scraper task failed")
        if not startup_success:
            print("   - Startup check task failed")
    print("=" * 70)

    return monthly_success and startup_success
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["-h", "--help"]:
        ...
        print("  1. Monthly scraper: Runs on the 15th of each month at 5:00 PM")
